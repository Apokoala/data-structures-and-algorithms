...
const addValues = (arr) => arr.reduce((partialSum, a) => partialSum + a, 0);
...
const addPurchases = (arr) => arr.reduce((partialSum, a) => partialSum + a.purchasePrice, 0);
...
const countNumberOfElements = (arr) => arr.reduce((partialSum) => partialSum +1, 0);
...
const returnNames = (arr) => {
  const name = arr.reduce((acc, names) =>{
    acc.push(names.name);
    return acc;
  }, []);
  return name;
