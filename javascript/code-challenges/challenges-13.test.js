...
------------------------------------------------------------------------------------------------ */
//My solution on line 11 does work in replit but the math.max method used below was from ezgi
//const longestString = (arr) => arr.indexOf(arr.reduce((a, b)=> a.length > b.length ? (a) : (b)));
const longestString = (arr) => arr.indexOf(arr.find(str => str.length === Math.max(...arr.map(str => str.length))));
...
const firstLetters = (arr) => arr.map(x => x[0]);
...
const allHappy = (arr) => arr.filter(str => str.includes(':)')).length === arr.length ? true : false;
...
const findEvery = (arr, target) => arr.filter(str => str.includes(target)).length === arr.length ? true : false;
...
const unenrollBrook = (arr) => arr.flatMap(newArr => 
